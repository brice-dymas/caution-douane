...
export class TypeVehiculeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.typeVehiculeService
            ...
            .subscribe(
                (res: HttpResponse<ITypeVehicule[]>) => this.paginateTypeVehicules(res.body, res.headers),
    ...
    trackId(index: number, item: ITypeVehicule) {
        return item.id;
    }

    registerChangeInTypeVehicules() {
        this.eventSubscriber = this.eventManager.subscribe('typeVehiculeListModification', response => this.loadAll());
    }
    ...
    protected paginateTypeVehicules(data: ITypeVehicule[], headers: HttpHeaders) {
